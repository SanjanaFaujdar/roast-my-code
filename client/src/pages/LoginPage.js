...
const LoginPage = () => {
  ...
  const handleSubmit = async () => {
    ...
    setError('');
    try {
      const res = await axios.post('https://roast-my-code-mbfw.onrender.com/api/auth/login', { email, password });
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0d1117', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: '#161b22', border: '1px solid #30363d', borderRadius: '12px', padding: '2rem', width: '100%', maxWidth: '400px' }}>
        <h2 style={{ color: '#ff6b35', textAlign: 'center', marginBottom: '1.5rem' }}>🔥 Login</h2>
